...
<div class="input-form">
	...
	<div class="row">
		...
		$sql	= 'SELECT * FROM %spa_photos WHERE member_id='.$_POST['member_id'];
		$row = queryDB($sql, array(TABLE_PREFIX, $_POST['member_id']), TRUE);


		$texto = 'Profile Picture'; 
		...
		$rutaFoto = "content/profile_pictures/originals/".$nombre_foto.""; 
		$ancho = 50;
		$alto= 50;	
		$border = 2.5;	

		echo "<img width=".$ancho." height=".$alto." src=".$rutaFoto." border =".$border.">";
	...
	<div class="row">
		...
		<label for="year"><?php echo _AT('year'); ?>: </label><input id="year" class="formfield" name="year" type="text" size="4" maxlength="4" value="<?php echo htmlspecialchars($_POST['year'], ENT_QUOTES); ?>" />  <label for="month"><?php echo _AT('month'); ?>: </label><input id="month" class="formfield" name="month" type="text" size="2" maxlength="2" value="<?php echo htmlspecialchars($_POST['month'], ENT_QUOTES); ?>" /> <label for="day"><?php echo _AT('day'); ?>: </label><input id="day" class="formfield" name="day" type="text" size="2" maxlength="2" value="<?php echo htmlspecialchars($_POST['day'], ENT_QUOTES); ?>" />
	</div>
